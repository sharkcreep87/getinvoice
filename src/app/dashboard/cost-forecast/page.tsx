...
export default function CostForecastPage() {
  ...
  }, [currentForecast?.ingredients])

  const calculateTotals = () => {
    if (!currentForecast) return

    const totalCost = currentForecast.ingredients.reduce(
      (sum, ing) => sum + ing.cost_per_unit_rm,
      0
    )

    const suggestedPrice = currentForecast.suggested_selling_price_rm
      ...
      {!currentForecast && (
        <Card className="border-2 border-primary/20 shadow-lg">
          <CardHeader className="bg-gradient-to-r from-primary/10 to-secondary/10 border-b border-primary/10">
            <CardTitle className="text-base sm:text-lg">Generate Cost Forecast</CardTitle>
            <CardDescription className="text-xs sm:text-sm">
              Enter a product name to get AI-generated ingredient breakdown and pricing
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-6">
            <form onSubmit={handleSubmit} className="flex gap-2">
              <Input
                ...
                placeholder="Product name (e.g., Karipap, Nasi Lemak, Iced Latte)"
                disabled={loading}
                className="flex-1"
              />
              <Button
